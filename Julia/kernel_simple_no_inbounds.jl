...
function cuGridKnnSimpleNoInbounds(Points, Queries,
    ...
    # startPoints::Int32 = 0
    # (threadIdx().x == 1)  && ( startPoints = IntPointsperblock[p_bid])
    # startPoints = shfl_sync(0xffffffff, startPoints, 1);
     startPoints::Int32 = IntPointsperblock[p_bid]
    startQueries::Int32 = IntQueriesperblock[q_bid]
    totalPoints::Int32 = Pointsperblock[p_bid]
    totalQueries::Int32 = Queriesperblock[q_bid]
    dist::Float32 = 0
    nb::Int32 = 0
    # 32 regs
    for q::Int32 = 0:stride:(totalQueries - 1)
        ...
        if tid + q <= totalQueries
            for d::Int32 = 1:dimensions
                    SharedQueries[d,tid] = Queries[qIndex, d]
            end
            dist = Distances[qIndex]
            nb = Neighbours[qIndex]
        end
        for p::Int32 = 0:stride:(totalPoints - 1)
            ...
            if tid + p <= totalPoints
                for d::Int32 = 1:dimensions
                    SharedPoints[d, tid] = Points[ startPoints + p + tid, d]
                end
            end
            sync_threads()
            ...
            for i::Int32 = 1:bounds
                tempdist::Float32 = 0
                for d::Int32 = 1:dimensions
                    tempdist += CUDAnative.pow(SharedQueries[d,tid]-SharedPoints[d, (i+tid-2)%bounds+1],2)
                end
        ...
        if tid + q <= totalQueries
            Distances[qIndex] = dist
            Neighbours[qIndex] = nb
        end
    end
    return nothing
end
...
function cuda_knn_simple_no_inbounds(OrderedPoints, OrderedQueries,
    PointsPerBlock, QueriesPerBlock,
    ...
    @cuda(blocks=(grids,grids,grids), threads=(32,thread_groups), shmem=shmem,
      cuGridKnnSimpleNoInbounds(devPoints, devQueries,
